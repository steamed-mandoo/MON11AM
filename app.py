...
image_paths = ['data/강문해변.png', 'data/경포해변.png', 'data/아르떼뮤지엄강릉.png', 
               ...
               'data/범바우막국수양양본점.png', 'data/설악케이블카.png', 'data/청초수물회속초본점.png', 
               ...
               'data/큰기와집[한식].png', 'data/파머스키친2호점.png', 'data/하조대.png'   
               ]
...
def additional_features_tab():
    return dcc.Tab(label='추가 설명', children=[
        ...
        html.Div([
            ...
            html.P("지방 인허가 로컬 데이터, 이동통신 데이터, 신용카드 데이터, 네비게이션 데이터, 관광객 방문 데이터 등의 다양한 데이터 소스를 활용"),
            html.P("또한 한국 관광 데이터 랩과 구글 키워드 분석도 활용"),
        ], style={'margin': '20px'}),
        ...
        html.Div([
            ...
            html.P("5) 각 관광지에 대한 키워드를 주관적으로 고려하여 추출한 후 태그(#)를 통해 연령별, 일행별, 활동별 키워드를 만들어 각 중심관광지와 결합"),
            html.P("6) 드롭다운을 이용해 지도에 중심관광지를 중심으로 연관관광지가 표시될 수 있도록 코드를 구성했으며, 중심관광지와 연관관광지의 마커 색깔은 다르게 구성하여 구별하기 편리하게 함"),
        ], style={'margin': '20px'}),

        html.Div([
            html.P("결론:", style={'color': 'blue', 'margin': '10px'}),
            html.P("결론적으로 MZ세대와 X세대의 방문객들의 선호지가 엄격히 강릉 또는 양양으로 분류되지 않았습니다."),
            html.P("저희가 전처리 한 자료를 통해 양양과 강릉 두 지역 모두 다양한 연령층과 일행의 관광객들이 많이 방문하고 있음을 확인할 수 있었습니다."),
            html.P("또한, 관광지에서 선호하는 활동 또한 다양하게 나타났음을 확인할 수 있었습니다."),
